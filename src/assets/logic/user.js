...
export async function Registered(walletAddress, selfAddress) {
  ...
  await selfweb3.GetWeb3().Execute(
    'call',
    'Registered',
    walletAddress,
    0,
...
function initBackend(flow, walletAddress, inputWeb2Key, callback, failed) {
  ...
  WasmPublic(function (wasmResponse) {
    ...
    selfweb3.httpGet(
      selfweb3.GetProps('ApiPrefix') + '/api/datas/load',
      queryMap,
      function (response) {
        if (
          response['Error'] !== '' &&
          ...
          WasmInit(
            walletAddress,
            inputWeb2Key,
            web2Response['Web2NetPublic'],
            web2Response['Web2Data'],
            function (initResponse) {
              ...
              if (
                ...
                selfweb3.ShowMsg('', flow, 'inint wasm successed', [
                  wasmResp['Data'],
                  web2Response['Web2Address'],
                ])
                if (callback !== undefined && callback !== null)
                  callback(wasmResp['Data'], web2Response['Web2Address'])
              }
            },
          )
        }
...
/* example
    ...
    // callback(qrcode)
*/
export function Register(walletAddress, selfAddress, recoverID, callback, failCb) {
  // wasm
  let userID = walletAddress
          ...
          verify3.WebAuthnRegister(
            ...
            function () {
              StoreSelfData(userID, recoverID, response['Data']['Web2Data'], function () {
                ...
                if (callback !== undefined && callback !== null)
                  callback(response['Data']['SelfAddress'], response['Data']['QRCode'])
              })
            },
            function () {
              failCb()
...
export function StoreSelfData(userID, recoverID, web2Data, callback) {
  ...
  selfweb3.httpPost(
    selfweb3.GetProps('ApiPrefix') + '/api/datas/store',
    formdata,
...
export function Reset(walletAddress, selfAddress, code, resetKind, callback, failCb) {
  if (resetKind === 'TOTP') {
    let verifyParams = { method: 'ResetTOTPKey', action: 'query', relateTimes: '1' }
    verify3.FinishEmailVerify(
      walletAddress,
      code,
      ...
      function (wasmResponse) {
        selfAuthVerify2(
          ...
          walletAddress,
          'query',
          wasmResponse['Data']['RelateVerify'],
          function () {
...
function selfAuthVerify2(flow, walletAddress, action, verifyParams, callback) {
  ...
  selfweb3.httpPost(
    selfweb3.GetProps('ApiPrefix') + '/api/user/auth/verify',
    formdata,
    function (response) {
      if (
        response['Error'] !== '' &&
        ...
        selfweb3.ShowMsg(
          'error',
          flow,
          'self authorization verify from web2 server failed',
          response['Error'],
        )
      } else {
      ...
      if (
        ...
        const merkleParams = selfweb3.GetWeb3().web3.eth.abi.encodeParameter(
          ...
          },
          {
            ...
            msgList: [
              Web3.utils.asciiToHex(verifyParams['message']),
              Web3.utils.asciiToHex(relateVerifyParams['message']),
            ],
